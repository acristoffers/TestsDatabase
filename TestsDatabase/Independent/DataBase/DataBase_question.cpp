...
int DataBase::question_insert(std::string title, std::string reference, int difficulty, std::string body, int category, int kind)
{
    ...
    sqlite3_stmt *stmt;
    sqlite3_prepare(_p->db, "INSERT INTO questions VALUES (:id, :title, :body, :difficulty, :reference, :category, :kind)", -1, &stmt, 0);
    SQL_ERROR
    ...
    SQL_ERROR
        sqlite3_bind_text(stmt, sql_param(":title"), title.c_str(), title.length(), SQLITE_STATIC);
    SQL_ERROR
        sqlite3_bind_blob(stmt, sql_param(":body"), blob, len, SQLITE_STATIC);
    SQL_ERROR
        sqlite3_bind_int(stmt, sql_param(":difficulty"), difficulty);
    SQL_ERROR
        sqlite3_bind_text(stmt, sql_param(":reference"), reference.c_str(), reference.length(), SQLITE_STATIC);
    SQL_ERROR
        sqlite3_bind_int(stmt, sql_param(":category"), category);
    SQL_ERROR
        sqlite3_bind_int(stmt, sql_param(":kind"), kind);
    SQL_ERROR
...
{
    std::string query = "SELECT id, title FROM questions";
...
bool DataBase::question_update(int id, std::string title, std::string reference, int difficulty, std::string body, int category, int kind)
{
    ...
    sqlite3_stmt *stmt;
    sqlite3_prepare(_p->db, query.c_str(), -1, &stmt, 0);
    SQL_ERROR
