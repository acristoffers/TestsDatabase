...
int DataBase::category_insert(std::string name, int parent)
{
    ...
    sqlite3_prepare(_p->db, "INSERT INTO categories VALUES (:id, :name, :parent)", -1, &stmt, 0);
    SQL_ERROR

        sqlite3_bind_int(stmt, sql_param(":id"), id);
    SQL_ERROR
        sqlite3_bind_text(stmt, sql_param(":name"), name.c_str(), name.length(), SQLITE_STATIC);
    SQL_ERROR
        sqlite3_bind_int(stmt, sql_param(":parent"), parent);
    SQL_ERROR
